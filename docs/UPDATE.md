...
1. A **CHANGELOG entry** (Keep-a-Changelog style: Added / Changed / Fixed /
   Migration notes) — read it *before* updating.
2. **Additive migrations**: new tables/columns/indexes only, applied with
   `prisma migrate deploy`. Destructive changes only ever appear in a MAJOR
   release, flagged loudly in Migration notes.
3. **No breaking env changes without a MAJOR bump**: new env vars in MINOR
...
4. Webhook topics / scopes / `shopify.app.toml` changes are called out
