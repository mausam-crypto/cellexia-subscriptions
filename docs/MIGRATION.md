...
2. **`customerPaymentMethodRemoteCreate`** (keep billing through Stripe): the
   ...
   Shopify and the `write_customers` scope (already in ours — Shopify gates payment-method mutations like customerPaymentMethodRemoteCreate and SendUpdateEmail behind write_customers, not a separate write scope).
