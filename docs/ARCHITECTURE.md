...
- **Jobs runner** (`app/lib/jobs/runner.server.ts`): registry entries flagged
  ...
  outcome promotion, no customer contact; every 10 minutes since v1.29.0), `stale_attempt_sweep`,
...
`whatsapp` (E.164), `chatUrl` (https), `hoursNote`, `slaBusinessDays` 1 (v1.29.0: replaced by `replyWithinValue` 30 / `replyWithinUnit` minutes / `alwaysOn` true — see below),
`requestsPerHour` 3 (format-refined at save with the resolver's own rules).

**Settings added in v1.29.0 (no migration).** `portal.singleSubscriptionOpensDetail`
true (home 302s a one-subscription customer to the detail page; `?list=1` is
the explicit list); `support.replyWithinValue` 30 (int ≥ 1; ceiling 10 080
minutes / 720 hours / 30 business days, `superRefine`), `support.replyWithinUnit`
`minutes` (`minutes | hours | business_days`), `support.alwaysOn` true —
together "A human replies within 30 minutes, 24/7."; `support.slaBusinessDays`
is tolerated on read only (a stored row without `replyWithin*` keys is read
as `{N, business_days, alwaysOn: false}` by the schema's `z.preprocess` and by
`resolveReplyPromise`; a merchant save drops it). Job cadence: `concierge_sla_run`
every 10 minutes. i18n keys: `portal.support.reply_promise.*`,
`portal.toast.support_sent_promise`, `cancel.saved.support_pending/support_hold`
(`{promise}`), `portal.value.milestone_away_one/_other`; the `_one/_other`
`sla` / `support_sent` / `support_pending` / `support_hold` keys are removed.
