...
`DATABASE_URL` is unset. The rest of this document is the *manual* E2E plan.
...
1. Reason `TOO_MUCH_PRODUCT` → expect skip/frequency-change saves offered
   (`cancel.save_shown`); accept one → `cancel.save_accepted`, outcome `SAVED`,
   contract still active, `savedAt` set.
2. Reason `TOO_EXPENSIVE` → discount save (default 15% × 2 cycles) → accept →
   verify a `DiscountGrant` row and that the **next renewal actually charges
   the discounted amount**.
3. Decline every save → final offer (default 25% × 2 cycles,
   `cancel.final_offer_shown`) → decline → `cancel.completed`, contract
   cancelled in Shopify, `winback.scheduled` event appears.
4. Re-run the flow on a contract that already used the final offer → the final
   offer must NOT be shown again (cooldown 180 days).
5. Count the steps to cancel when declining everything: must be **≤ 3 clicks**
   ...
   halfway → outcome `ABANDONED`, contract untouched.
...
3. Open the reactivation link → reactivate → `winback.reactivated`, new/resumed
   contract active, win-back state `WON_BACK`.
4. Let another one run to `winback.sunset` → no further touches ever. Restore
   the real offsets.
...
Before every release ([UPDATE.md](./UPDATE.md)): `npm test` + `npm run
typecheck` green, then re-run §3 rows 1/2/12/16, one §4 recovery, and one
