...
export class ElementNode {
  ...
  /* Writable, like the DOM. The diagnostic cards both asset files build set
     `element.className = '…'`; a getter-only property makes that assignment
     throw under 'use strict', and both call sites swallow their exceptions on
     purpose (a diagnostic is never worth breaking a page over), so the shim
     would have silently reported "no card" for code that works in a browser. */
