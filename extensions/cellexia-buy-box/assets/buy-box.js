...
(function () {
  ...
  function previewBoot() {
    ...
    if (!document.querySelector(OWN_GATED)) {
      return;
    }
   ...
   *  'foreign' — its [name="id"] holds an id that is not ours.
   *
  ...
  /** First provably-ours form, else the first one we cannot disprove. */
  ...
  function findProductForm(root, variants) {
    ...
    if (scoped.length) {
      /* Inside the block's own section the placement itself is the ownership
         proof (the app block renders in the product section), so the scoped
         form is still used when the identity check comes back inconclusive —
         e.g. a theme that stores something other than a variant id in
         [name="id"]. Behaviour for the app-block install is unchanged. */
