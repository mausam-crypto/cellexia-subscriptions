...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // A bare-timestamp cursor with strict lt/gt silently skipped every event
  // sharing the boundary millisecond (webhook bursts write several rows in
  // one ms), and single-key ordering let equal-timestamp rows swap between
  // page queries. The cursor condition is ANDed alongside the filter where,
  // never merged into it: `where` may already carry its own createdAt range
  // (date filter) and OR legs.
  ...
  return json({
    ...
    nextCursor: events.length
      ? encodeCreatedIdCursor(events[events.length - 1])
      : null,
    prevCursor: events.length ? encodeCreatedIdCursor(events[0]) : null,
    events: events.map((e) => ({
      id: e.id,
      type: e.type,
      source: e.source,
...
// csvEscape lives in ~/lib/csv.server: shared with the subscribers export so
