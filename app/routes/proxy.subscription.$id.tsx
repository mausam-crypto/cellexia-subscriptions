...
function scheduleHtml(
  ...
  const maxDate = dateInputValue(addDaysTz(now, NEXT_DATE_MAX_DAYS, tz), tz);
  ...
  const skipForm = `<form method="post" action="${api(ctx, "skip")}">${hiddenFields(baseFields(ctx))}<button type="submit" class="cx-btn cx-btn--quiet cx-btn--small">${escapeHtml(t(locale, "portal.actions.skip"))}</button></form>`;
  const delayForm = `<form method="post" action="${api(ctx, "delay")}" class="cx-row cx-row--wrap">${hiddenFields(baseFields(ctx))}
