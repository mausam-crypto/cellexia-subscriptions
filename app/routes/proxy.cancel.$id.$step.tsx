 ...
 * config.server.ts). Max 2 concrete one-tap cards; the deeper final offer is
 * reserved for the very last moment and is honestly one-time (cooldown
 * enforced in eligibleForFinalOffer). Never showing generic discounts before
 * knowing the reason prevents discount training.
 *
 * Compliance (FTC click-to-cancel): skipping every offer, cancellation
 * completes in ≤3 required clicks — "Continue to cancel" (step 1) → reason
 * submit (step 2) → "No thanks, cancel my subscription" (step 3), which
 * POSTs the confirm action and completes immediately. The final offer, when
 * eligible, interjects exactly once and its decline cancels immediately —
 * never a loop, never a hidden link — and confirm/done state exactly what
 * happens next (no more charges, resume anytime).
 */
...
async function loadSaves(
  ...
  if (ctx.portalSession.isPreview) {
    ...
    // no recordSaveShown, and the exit goes straight to the final-offer page.
  ...
  const offers = await getSavesForReason(shop.id, reason.key, contract);
  if (offers.length === 0) {
    // Nothing applicable to offer — go straight toward the exit.
    return redirect(
      toPath(
        ctx,
        (await eligibleForFinalOffer(contract.id)) ? "final" : "confirm",
      ),
    );
  }
...
async function loadDone(ctx: CancelRouteContext) {
  ...
  return renderCancelPage(ctx, pageDone({ locale: ctx.locale }));
}
...
/**
 * The terminal action. Interjects the final offer at most once (only when
 * eligible AND a reason is known AND it has not been shown yet — reason-gated,
 * never a loop); otherwise completes the cancellation immediately. A missing
 * session never blocks cancellation: one is created on the spot.
 */
