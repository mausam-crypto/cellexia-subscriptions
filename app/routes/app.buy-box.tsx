...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [draftOrPublished, revisions, performance, launch] = await Promise.all([
...
const TEXT_FIELDS: { key: TextKey; label: string; templated: boolean }[] = [
  ...
  { key: "firstOrderLine", label: "First-order note", templated: true },
  {
    ...
    label: "One-time link label (Value stack)",
    templated: true,
  },
];
...
/** Layout knobs a preset's archetype requires when switching to it. */
...
export default function BuyBoxDesignerPage() {
  ...
  return (
    <Page
      ...
      subtitle="Six conversion-tested presets with deep customization — preview everything, publish safely, restore instantly."
      primaryAction={{
        content: "Publish",
