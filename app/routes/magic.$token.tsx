...
(function () {
  ...
  return pageShell(desc.title, body);
}
...
function successPage(result: MagicActionResult): string {
  ...
  return pageShell(result.headline, `<div class="card">${parts.join("\n  ")}</div>`);
}
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (result.redirect) {
      ...
      // so the Shopify card page can be revisited from the same email.
      const headers = new Headers();
      if (result.setCookie) headers.append("Set-Cookie", result.setCookie);
      return redirect(result.redirect, { status: 303, headers });
    }

    return html(successPage(result), 200, result.setCookie);
  } catch (err) {
