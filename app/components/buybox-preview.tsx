 ...
 * context.
 *
 * v1.2.0: honors layout.showFrequency=false in all presets — the
 * frequency control disappears and the planner shows a single
 * recommended-cadence line instead of its chips.
 *
 * v1.6.0: subscription_max — the subscription card IS the buy box, one-time
 * demoted to a quiet priced link below it. The replica renders both halves
 * of the link's state machine via `selected`: subscription selected → the
 * quiet "or buy once for {amount}" link; one-time selected → the link's
 * minimal selected state (small check + "One-time purchase — {amount}" +
 * a quiet switch-back link). The switch-back copy is an English
 * approximation of the extension's t key. Note the preset's calm defaults
 * (no badge, no frequency selector, empty heading) are applied to the
 * config when the merchant SELECTS the preset (PRESET_LAYOUT_PATCH in
 * app.buy-box.tsx), not forced here — a per-market subscription_max
 * override inherits the base layout, and this replica mirrors that
 * storefront behavior exactly.
 */
...
const EN = {
  ...
  switchBack: "Switch back to Subscribe & Save",
...
export function BuyBoxPreview({
  ...
  switch (cfg.preset) {
    ...
    case "subscription_max": {
      // The subscription card IS the buy box; the one-time purchase is a
      // quiet priced link BELOW it (never emphasized, price visible BEFORE
      // selection). Selecting one-time swaps the link into a minimal
      // selected state + a quiet switch-back line. Mirrors the
      // cx-buybox__submax-* markup in cx-buybox-core.liquid; this preset
      // deliberately ignores layout.order — one-time always sits below.
      const quietSavings = layout.showSavings ? (
        <span className="cx-buybox__save cx-buybox__save--quiet">
          {saveStr}
        </span>
      ) : null;
      body = (
        <div className="cx-buybox__group cx-buybox__group--submax">
          <div
            className={`cx-buybox__option cx-buybox__option--sub cx-buybox__submax-card${isSub ? " is-selected" : ""}`}
          >
            {badgePill}
            <span className="cx-buybox__card">
              <span className="cx-buybox__card-body cx-buybox__card-body--stack">
...
/* subscription_max — the subscription card IS the buy box; the one-time
   purchase is a single muted underlined line BELOW it with generous
   whitespace (quiet, never hidden: its price is in the link text). The
   selected state swaps the link for a small check + label + price plus a
   quiet switch-back line. Class names and values mirror the
   cx-buybox__submax-* rules in assets/buy-box.css (the interactive
   display toggles become the .is-selected conditional render here). */
.cx-buybox__submax-card .cx-buybox__save--quiet {
  font-weight: 400;
}

/* The reassurance line is load-bearing here — full text color, not muted. */
.cx-buybox__submax-card .cx-buybox__reassurance {
  color: var(--cx-text, inherit);
  font-weight: 400;
}

/* Generous whitespace above the demoted line (on top of the group gap). */
.cx-buybox__submax-onetime {
  margin-block-start: 10px;
  text-align: start;
}

.cx-buybox__submax-link {
  display: inline-flex;
  align-items: center;
  min-block-size: 44px;
  padding: 10px 2px;
  font-size: calc(13px * var(--cx-font-scale, 1));
  font-weight: 300;
  line-height: 1.4;
  color: var(--cx-muted);
  text-decoration: underline;
...
.cx-buybox__submax-picked .cx-buybox__check {
  align-self: center;
  margin-block-start: 0;
}

.cx-buybox__submax-picked-price {
  font-weight: 400;
  font-variant-numeric: tabular-nums;
  white-space: nowrap;
}

.cx-buybox__submax-switchback {
  display: inline-flex;
  align-items: center;
  min-block-size: 44px;
  padding: 10px 2px;
