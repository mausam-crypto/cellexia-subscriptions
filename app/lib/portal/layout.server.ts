...
const SCRIPT = `
(function(){
  // Magic-link LOGIN hand-off: persist the ?session= token as the raw portal
  // cookie (validated server-side by hash lookup) and scrub it from the URL.
  var m=location.search.match(/[?&]session=([^&]+)/);
  if(m){
    document.cookie="cellexia_portal_session="+m[1]+"; Path=/apps/cellexia-subscriptions; Max-Age=2592000; Secure; SameSite=Lax";
...
export function portalPage(input: PortalPageInput): string {
  ...
  const back = input.backHref
    ? `<a class="cx-back" href="${escapeHtml(input.backHref)}">&larr; ${escapeHtml(input.backLabel ?? t(locale, "portal.back"))}</a>`
    : "";
