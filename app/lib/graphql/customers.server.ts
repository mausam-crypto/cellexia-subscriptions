...
export interface ShopifyCustomer {
  ...
  phone: string | null;
  locale: string | null;
  /** Account creation instant — feeds acqTimeToPurchaseSeconds (browse-to-buy). */
  createdAt: Date | null;
  /** Lifetime order count on Shopify (acquisition context; null when unreadable). */
...
function normalizeCustomer(raw: RawCustomer): ShopifyCustomer {
  ...
  const ordersNum =
    raw.numberOfOrders == null ? NaN : Number(raw.numberOfOrders);
  return {
    id: raw.id,
    ...
    numberOfOrders: Number.isFinite(ordersNum) ? ordersNum : null,
    tags: raw.tags ?? [],
