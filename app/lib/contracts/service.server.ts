...
export async function addLine(
  ...
  const created = await withMirrorGuard("addLine", ctx, options, () =>
...
export async function removeLine(
  ...
  if (line.shopifyLineId) {
    ...
    try {
      if (line.isOneTimeAddon) {
        // Addons live on the next billing cycle, not on the contract.
        const nextBillingDate = requireNextBillingDate(ctx);
        const cycle = await getBillingCycleByDate(
          admin,
          contract.shopifyContractId,
...
export async function addOneTimeAddon(
  ...
  const created = await withMirrorGuard("addOneTimeAddon", ctx, options, () =>
    prisma.contractLine.create({
      data: {
        contractId: contract.id,
        shopifyLineId,
...
/**
 * Resume a paused contract and bill soon (next billing date = now + 3 days),
 * so the customer who asked to come back gets product quickly rather than
...
export async function resumeContract(
  ...
  await logEvent({
    ...
    source: resolveSource(options),
    actor: resolveActor(options),
    payload: { nextBillingDate: effectiveNext.toISOString() },
  });
...
const GRANT_EVENT_TYPES: Record<string, string> = {
  ...
  WINBACK: "winback.discount_offered",
