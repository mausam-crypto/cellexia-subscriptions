 ...
 *
 * ── Compliance guardrails (FTC click-to-cancel) ──────────────────────────────
 ...
 * - The final offer (step 4) is shown at most once per flow, only when the
 *   cooldown allows it, and its decline control cancels immediately — it adds
 *   at most one optional click and never loops.
 ...
 *   the copy ("one-time offer", milestone progress) is enforced by data.
 */
...
/** At most this many reason-matched saves are shown on step 3. */
export const MAX_SAVES_SHOWN = 2;

/** FREQUENCY save suggests shipping this many weeks less often. */
export const FREQUENCY_SUGGEST_DELTA_WEEKS = 2;

/** PAUSE save suggests this many months (clamped by settings.pause.maxMonths). */
 ...
 * starts fresh (and startCancelSession marks stale ones ABANDONED).
 */
