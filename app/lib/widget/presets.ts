 ...
 * publish. The CSS is injected scoped inside the widget wrapper only, but is
 * still merchant-authored content on the storefront, so: strip `@import`,
 * `expression(`, `url(` with any non-relative scheme, `<`, `>` and
 * `javascript:`; cap at 5000 chars. Stripping loops until stable so removed
 * fragments cannot recompose a banned token (e.g. "@imp@importort").
 */
export function sanitizeCustomCss(css: string): string {
  ...
  return out.slice(0, CUSTOM_CSS_MAX_LENGTH);
}
